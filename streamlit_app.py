...
loaded_model = pickle.load(open('C:\Users\pooja\PycharmProjects\pythonProject\heart_disease_model.pkl', 'rb'))

def predict_heart_disease(sample_patient_features):

    # Convert the sample to a DataFrame or appropriate input format
    input_df = pd.DataFrame([sample_patient_features], columns= X_train.columns)
