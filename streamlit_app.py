...
def predict_heart_disease(sample_patient_features):
    try:
        input_df = pd.DataFrame([sample_patient_features], columns=[ 'age', 'sex', 'trestbps', 'chol', 'fbs', 'thalach',
            'exang', 'oldpeak', 'slope', 'ca', 'cp_1', 'cp_2',
            'cp_3', 'restecg_1', 'restecg_2', 'thal_1', 'thal_2', 'thal_3'])  # Add all feature names
...
def main():
    ...
    if submit_button:
        ...
        # Ensure the button correctly references the input
        if st.button("Predict"):
            try:
                prediction = predict_heart_disease(sample_patient_features)  # Pass input features here
